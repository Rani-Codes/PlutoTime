    ...
    <div className="animate-fade-up flex w-full max-w-md flex-col items-center gap-8 text-center">
      ...
      {brightnessRatio !== null && (
        <div className="relative w-full">
          <div className="animate-glow-pulse absolute -inset-1 rounded-2xl bg-gold opacity-10 blur-lg" />
          <div className="relative rounded-2xl border border-white/10 bg-space-900/85 px-6 py-5 backdrop-blur-sm">
            <p className="font-mono text-4xl font-bold tabular-nums text-gold">
...
function ResultCard({
  ...
  const badgeColor = theme === 'dawn' ? 'bg-dawn' : 'bg-dusk'
  ...
  return (
    <div className="group relative rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-5 backdrop-blur-sm transition-colors hover:border-white/20">
      <div className={`mx-auto mb-2 flex size-7 items-center justify-center rounded-full ${badgeColor}`}>
        <SunGlyph />
      </div>
